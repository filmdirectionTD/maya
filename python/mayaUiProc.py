...
def getMayaSceneName ( withoutSubdir = True, withoutExt = True ) :
  #
...
def getMayaRenderEngine():
  return 0

def getMayaRenderSequence():
  name = cmds.getAttr('defaultRenderGlobals.imageFilePrefix')
  return name

def getMayaImageRenderFormat():
  imgFormat = cmds.getAttr('defaultRenderGlobals.imageFormat')
  return 'exr'

def getMayaOutputRenderDir():
  renderDir = os.path.join(cmds.workspace(q=True, rootDirectory=True )[:-1],
                           cmds.workspace(fileRuleEntry='images'))
  return renderDir.replace('\\', '/')

def getRenderImageOptions():
  firstFrame = cmds.renderSettings(firstImageName=True)
  lastFrame = cmds.renderSettings(lastImageName=True)
  imageName, padding, extension = firstFrame[0].split('.')
  padding = '%' + str(len(padding)).zfill(2) + 'd'
  #print 'HAHAHA' + imageName, padding, extension
  return imageName, padding, extension

def getMayaRenderCamera():
  return 0
